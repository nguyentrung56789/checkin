...
/* ===== KẾT NỐI CONFIG.JS =====
   ...
   app_checkin.js chỉ đọc cấu hình qua:
   - window.configReady
   - window.getConfig(...)
================================ */

const SHEET_URL_PARAM =
  new URLSearchParams(location.search).get('sheet');

let SHEET_URL = '';

window.appCheckinConfigReady = (async () => {
  if (!window.configReady) {
    throw new Error(
      'Không tìm thấy configReady. Kiểm tra thứ tự tải internal_key.js, config.js và app_checkin.js.'
    );
  }
  ...
  if (typeof window.getConfig !== 'function') {
    throw new Error(
      'Không tìm thấy getConfig() từ config.js.'
    );
  }
   ...
   * Ưu tiên:
   * 1. ?sheet= trên URL
   * 2. csv_url từ config.js
   */
  SHEET_URL =
    SHEET_URL_PARAM ||
    window.getConfig('csv_url') ||
    '';
...
async function ensureSheetPoints() {
  await window.appCheckinConfigReady;

  if (!SHEET_URL) {
    throw new Error('Thiếu csv_url trong config.js');
  }

  if (SHEET_POINTS.length) {
    return SHEET_POINTS;
  }
  ...
  if (!res.ok) {
    throw new Error(
      `Không tải được Sheet. HTTP ${res.status}`
    );
  }
  ...
  try {
    ...
    if (
      ...
      if (hit) {
        ...
        toast(
          `✅ ${label} đã được check-in (${hit.dist}m)`,
          'ok',
          3500
        );
      }
...
if (!MA_NV) {
  try {
    alert(
      'Thiếu mã nhân viên. Vui lòng vào từ Trang chính.'
    );
  } catch {}
...
if (
  String(LOGGED_NV.ma_nv) !==
  String(MA_NV)
) {
  try {
    alert(
      'Mã nhân viên không khớp tài khoản đăng nhập.'
    );
  } catch {}
...
async function ensureGeoAllowedAndGet() {
  if (!('geolocation' in navigator)) {
    toast(
      'Thiết bị không hỗ trợ vị trí (GPS)',
      'err',
      3500
    );
  ...
  const st =
    await getGeoPermStateSafe();

  if (st === 'denied') {
    toast(
      '⚠️ Bạn đã chặn quyền vị trí. Hãy bật lại trong cài đặt trình duyệt.',
      'err',
      4200
    );
  ...
  if (r && r.err) {
    ...
    toast(
      '⚠️ Không lấy được GPS. Hãy bật vị trí/GPS và thử lại.',
      'err',
      3500
    );
  ...
  if (
    ...
    toast(
      '⚠️ Chưa có vị trí GPS. Hãy bật vị trí rồi thử lại.',
      'err',
      3500
    );
    ...
    try {
      alert(
        '⚠️ Chưa có vị trí GPS. Hãy bật vị trí/GPS rồi bấm lại "Bật camera".'
      );
    } catch {}
  ...
  try {
    ...
    try {
      ...
      if (e.name === 'NotAllowedError') {
        throw new Error(
          'Bạn đã chặn quyền camera. Vào Cài đặt trình duyệt để bật lại.'
        );
      }
    ...
    toast(
      'Đã bật camera. GPS sẽ lấy khi bấm chụp.',
      'ok'
    );
  } catch (e) {
    console.error(e);
...
async function setZoom(next) {
  ...
  next = Math.max(
    zoomMin,
    Math.min(zoomMax, next)
  );

  if (
    Math.abs(next - zoomVal) <
    1e-3
  ) {
    return;
  }
...
if (btnShot) {
  btnShot.onclick = async () => {
    ...
    try {
      ...
      const gps =
        await getGPSOnce(10000);

      if (gps && gps.err) {
        if (gps.err.code === 1) {
          toast(
            '⚠️ Bạn đã chặn quyền vị trí. Hãy bật lại GPS.',
            'err',
            4200
          );
        ...
        toast(
          '⚠️ Không lấy được GPS. Hãy bật vị trí rồi chụp lại.',
          'err',
          3500
        );
      ...
      if (
        ...
        toast(
          '⚠️ Chưa có vị trí. Hãy bật GPS rồi chụp lại.',
          'err',
          3500
        );
        ...
        try {
          alert(
            '⚠️ Chưa có vị trí. Hãy bật GPS/vị trí rồi chụp lại.'
          );
        } catch {}
      ...
      sessionStorage.setItem(
        'CHECKIN_CURRENT_SHOT_ID',
        shotId
      );

      sessionStorage.removeItem(
        'CLICKED_CHECKIN_BROWSER_V1'
      );

      sessionStorage.removeItem(
        'CHECKIN_BROWSER_DONE_V1'
      );
...
function showStage() {
  if (
    stage &&
    !stage.classList.contains('ready')
  ) {
    stage.classList.add('ready');
  }
}
...
(async () => {
  try {
    /*
     * Chờ config.js hoàn tất trước khi khởi động.
     */
    await window.appCheckinConfigReady;
    ...
    toast(
      'Bấm nút "Cam" để mở camera.',
      'info',
      2200
    );
  } catch (error) {
    console.error(
      'Lỗi khởi tạo app_checkin:',
      error
    );
  ...
  () => {
    if (document.hidden) {
      ...
      if (!stream) {
        ...
        toast(
          'Bấm nút "Cam" để mở lại camera. GPS sẽ lấy khi chụp.',
          'info',
          2600
        );
      }
